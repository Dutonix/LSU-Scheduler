...
public:
    ...
    DataStore(
    const std::vector<CourseCatalog>& catalogData,
    const std::vector<MetaData>& metaData,
    const std::vector<DegreeAudit>& degreeData)
    : catalog_Data(catalogData), meta_Data(metaData), degree_Data(degreeData) {}

    void load_catalog_Data() 
    {
            try {
        json jsonData = readJsonFromFile("../code/course_data.json");

        for (const auto& courseData : jsonData) 
        {
    ...
    {
            try {
        json jsonData = readJsonFromFile("../code/course_meta.json");
    ...
    void load_degree_Data() {
            try {
        json jsonData = readJsonFromFile("../code/degree_audit_data.json");

        for (const auto& courseData : jsonData) {
            DegreeAudit course(
                courseData.value("dept", ""),
                courseData.value("course_number", 0),
                courseData.value("complete", false));
    ...
    //Format the catalog data for printing
    

    // Prints the catalog data
    void print_catalog() const {
        for (const auto& course : catalog_Data) {
    ...
    void print_meta() const {
        for (const auto& course : meta_Data) {
            std::cout << course << std::endl;
        }
    }

    // Prints the degree data
    void print_degree() const {
        for (const auto& course : degree_Data) {
            std::cout << course << std::endl;
        }
    }
